...
<form method="post"><?
	if ($entries) {
		foreach ($entries AS $key => $objTranslation) {
			?>
			<div>
				<label><?= $objTranslation->original ?></label>
				<input type="hidden" name="translation<?= $key ?>[]" value="<?= $objTranslation->context ?>" />
				<textarea name="translation<?= $key ?>[]"><?= $objTranslation->original ?></textarea>
				<textarea name="translation<?= $key ?>[]"><?= $objTranslation->translation ?></textarea>
				<?if($objTranslation->hasPlural()){?>
					<textarea name="translation<?= $key ?>[]"><?= $objTranslation->plural ?></textarea>
					<textarea name="translation<?= $key ?>[]"><?= (!empty($objTranslation->pluralTranslation)) ? $objTranslation->pluralTranslation[0] : '' ?></textarea>
				<?}?>
			</div><?
		}
	}
